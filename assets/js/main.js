...
function initSwiper() {
  ...
  try {
    const swiperTools = new Swiper(".swiper-wrapper > .elementor-widget-wrap", {
      slidesPerView: 1,
      loop: true,
      speed: 500,
      spaceBetween: 0,
    ...
    if (сurrencyData.length !== 0) {
      ...
      if (chartArr.length === 0) {
        getCtx(newCurrencyData).forEach((ctx, id) => {
          ...
          var coinRates = document.querySelectorAll(`.coinschart .coins-rate-table-desktop-num`);
          // If first character is '+' the chart should be green. Example: '+1.34%'
          var isGreen = coinRates && coinRates[id] && coinRates[id].innerHTML[0] === "+";
